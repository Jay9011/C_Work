...
int main(int argc, char** argv)
{
	...
	// %p 로 출력시 16진수 8자리로 표현 (32bit, 4byte) ※ 16진수 2자리를 1byte 분량에 할당
	printf("n = %d\nn의 주소 : %d, %x, %p\n", n, &n, &n, &n);
	...
	*/
	...
	/* 참조연산자 *
		포인터를 사용해서 담고 있는 주소값을 찾아가 그 주소 안의 '값' 을 꺼내는 것.
	*/
	printf("*p = %d\n", *p);
	...
	// ※ 포인터변수에 직접 숫자를 대입하는 것은 !위험! 하다. (실제 주소를 참조하기 때문에 메모리가 망가질 수 있다)
	*p = 500;	// p가 가리키는 주소(int)에 500을 대입...
	printf("*p2 = %d, *p = %d, n = %d\n", *p2, *p, n);

	p = 100;	// p가 가리키는 주소를 100으로 바꿈...
	//printf("p = %d\n", *p);	// 접근하면 안되는 메모리 영역을 포인터로 접근하려고 하면 예외가 발생함.
